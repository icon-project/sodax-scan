...
const SELECTORS = {
    ...
    createIntent: calculateSelector(
        'createIntent((uint256,address,address,address,uint256,uint256,uint256,bool,uint256,uint256,bytes,bytes,address,bytes))'
    ),
    ...
    switch (selector) {
        case SELECTORS.supply: {
            ...
            return {
                ...
                amount: tokenAmount,
                actionText:
                    tokenAddress in assetsInformation
                        ? `Supply ${bigintDivisionToDecimalString(supply[1], assetsInformation[tokenAddress].decimals)} ${assetsInformation[tokenAddress].name}`
                        : `Supply ${bigintDivisionToDecimalString(tokenAmount, 18)} ${tokenAddress}`
            }
        }
        case SELECTORS.withdraw: {
            ...
            return {
                ...
                actionText:
                    tokenAddress in assetsInformation
                        ? `Withdraw ${bigintDivisionToDecimalString(withdraw[1], assetsInformation[tokenAddress].decimals)} ${assetsInformation[tokenAddress].name}`
                        : `Withdraw ${bigintDivisionToDecimalString(withdraw[1], 18)} ${tokenAddress}`
            }
        }
        case SELECTORS.borrow: {
            ...
            return {
                ...
                actionText:
                    tokenAddress in assetsInformation
                        ? `Borrow ${bigintDivisionToDecimalString(borrow[1], assetsInformation[tokenAddress].decimals)} ${assetsInformation[tokenAddress].name}`
                        : `Borrow ${bigintDivisionToDecimalString(borrow[1], 18)} ${tokenAddress}`
            }
        }
        case SELECTORS.repay: {
            ...
            return {
                ...
                actionText:
                    tokenAddress in assetsInformation
                        ? `Repay ${bigintDivisionToDecimalString(repay[1], assetsInformation[tokenAddress].decimals)} ${assetsInformation[tokenAddress].name}`
                        : `Repay ${bigintDivisionToDecimalString(repay[1], 18)} ${tokenAddress}`
            }
        }
        ...
        case SELECTORS.deposit: {
            ...
            return {
                ...
                actionText:
                    tokenAddress in assetsInformation
                        ? `Deposit ${bigintDivisionToDecimalString(deposit[1], assetsInformation[tokenAddress].decimals)} ${assetsInformation[tokenAddress].name}`
                        : `Deposit ${bigintDivisionToDecimalString(deposit[1], 18)} ${tokenAddress}`
            }
        }
        case SELECTORS.swap: {
...
export const parseSolanaTransaction = async (txnHash: string, connSn: string): Promise<string> => {
    ...
    const response = (await axios.post(process.env.RELAY_URL || '', data)).data
...
/**
 * Parse payload data, includes fallback try, if fallback fails, returns 'SendMessage' action type
 */
export const parsePayloadData = (data: string, srcChainId: string, dstChainId: string): actionType => {
    ...
    try {
        ...
        if (Array.isArray(rlp) && rlp.length === 5) {
            ...
            const tokenAmount = rlp[3] && (rlp[3] as Uint8Array).length > 0 ? BigInt(`0x${Buffer.from(rlp[3] as Uint8Array).toString('hex')}`) : 0n
...
function processAmount(amountStr: string, scale: number): string {
    ...
    const fractionalPart = ((remainder * BigInt(10 ** 18)) / divisor).toString().padStart(18, '0').replace(/0+$/, '')

    return fractionalPart ? `${quotientStr}.${fractionalPart}` : quotientStr
}

function decodeTokenAddress(tokenAddress: string, srcChainId: string, dstChainId: string): string {
